/**
 * app/api/manga/new/route.js
 * New series manga — Komikstation only
 *
 * NEW SERIES:
 *   GET /api/manga/new
 *   GET /api/manga/new?page=2
 *
 * Source: https://komikstation.org
 * URL    : https://komikstation.org/manga/?type=manga&order=latest
 */
...
export async function GET(req) {
  ...
  const cacheKey = `manga:new:komikstation:${page}`;
  const hit = await cacheGet(cacheKey);
  if (hit) return successResponse(hit, { fromCache: true });

  try {
    const results = await fetchNewSeriesKomikstation(page);
    const payload = { page, source: 'komikstation', mode: 'new', total: results.length, results };
    await cacheSet(cacheKey, payload, 300); // cache 5 menit
    return successResponse(payload);
  } catch (err) {
    console.error('[manga/new][komikstation]', err.message);
    return gatewayError(`Gagal mengambil seri baru manga: ${err.message}`);
  }
}
...
// ─────────────────────────────────────────────────────────────────
// NEW SERIES — https://komikstation.org/manga/?type=manga&order=latest
// Note: order=latest di sini mengurutkan berdasarkan seri terbaru
//       (berbeda dengan order=update yang mengurutkan update chapter)
// ─────────────────────────────────────────────────────────────────

async function fetchNewSeriesKomikstation(page) {
  const target = new URL('https://komikstation.org/manga/');
  target.searchParams.set('type', 'manga');
  target.searchParams.set('order', 'latest');
