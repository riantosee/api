/**
 * app/api/manga/popular/route.js
 * Popular manga — Komikstation only
 *
 * POPULAR:
 *   GET /api/manga/popular
 *   GET /api/manga/popular?page=2
 *
...
export async function GET(req) {
  ...
  const cacheKey = `manga:popular:komikstation:${page}`;
  const hit = await cacheGet(cacheKey);
  if (hit) return successResponse(hit, { fromCache: true });

  try {
    const results = await fetchPopularKomikstation(page);
    const payload = { page, source: 'komikstation', mode: 'popular', total: results.length, results };
    await cacheSet(cacheKey, payload, 600); // cache 10 menit (data popular lebih stabil)
    return successResponse(payload);
  } catch (err) {
    console.error('[manga/popular][komikstation]', err.message);
    return gatewayError(`Gagal mengambil popular manga: ${err.message}`);
  }
}
...
// ─────────────────────────────────────────────────────────────────
// POPULAR — https://komikstation.org/manga/?status=&type=manga&order=popular
// ─────────────────────────────────────────────────────────────────

async function fetchPopularKomikstation(page) {
