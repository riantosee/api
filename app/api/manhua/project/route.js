/**
 * app/api/manhua/project/route.js
 * Scraper "Project Update" — manhwaindo.my
 *
 * GET /api/manhua/project        → halaman 1
 * GET /api/manhua/project?page=2 → halaman berikutnya
 */
...
export async function GET(req) {
  ...
  const cacheKey = `manhua:project:${page}`;
  ...
  try {
    ...
    const results = page > 1
      ? parseUtaItems(html)
      : extractSection(html, 'Project Update');

    if (!results.length) return errorResponse(404, 'Data project update tidak ditemukan.');

    await cacheSet(cacheKey, results, 300);
    return successResponse(results, { total: results.length, page, source: 'manhwaindo.my' });
  } catch (err) {
    return gatewayError(`Gagal ambil project update: ${err.message}`);
  }
}
